...
namespace MarcoParenzan.Office365.API.Controllers
{
    ...
    public class Office365Controller : Controller
    {
        ...
        internal static string GetAuthorizationUrl(string resourceId, Uri redirectTo)
        {
            ...
            //     individual controllers, the Office365Controller will handle the entire auth flow,
            //     and only redirect to the original caller on completion.
            Uri redirectToThisController = new Uri(context.Request.Url, "/Office365");
        ...
        public ActionResult Index(string code, string error, string error_description, string state)
        {
            ...
            try
            {
                ...
                SaveInCache("UserId", result.UserInfo.UniqueId);
        ...
        public static ActionResult ShowErrorMessage(string message, string errorDetails)
        {
            IController commonController = new DefaultControllerFactory().CreateController(
                ...
                "Office365");
            return ((Office365Controller)commonController).ShowErrorMessageHelper(message, errorDetails);
        }
